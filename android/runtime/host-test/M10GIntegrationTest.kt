...
private inline fun expectFailure(block: () -> Unit) {
    ...
    try {
        block()
    } catch (_: VN97CapabilityActivationException) {
        failed = true
    }
    check(failed)
}
...
fun main() {
    ...
    val staged = VN97StagedCapability(
        parsed,
        VN97CapabilitySignatureEnvelope(),
        packageFile,
        packageFile,
    )
    ...
    check(
        profile.formatRules.single().acceptedFormats ==
            listOf("VN97MI1")
    )
    ...
    check(
        backend.inspect(prepared.token).state ==
            VN97BackendTransactionState.PREPARED
    )
    check(validations == 1)

    val reopened = VN97ModelImageActivationBackend(
        root,
        VN97ModelImageCandidateValidator { file, _ ->
            check(file.readBytes().contentEquals(image))
        },
    )
    check(
        reopened.inspect(prepared.token).state ==
            VN97BackendTransactionState.PREPARED
    )
    ...
    val restarted = VN97ModelImageActivationBackend(
        root,
        VN97ModelImageCandidateValidator { _, _ -> },
    )
    check(restarted.commit(prepared.token) == revision)

    restarted.rollback(prepared.token)
    check(
        restarted.inspect(prepared.token).state ==
    ...
    tampered[prefix.size] =
        (tampered[prefix.size].toInt() xor 1).toByte()
    packageFile.writeBytes(tampered)
    expectFailure {
        restarted.prepare(verified, plan)
    }
